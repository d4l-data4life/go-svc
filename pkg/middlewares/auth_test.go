...
func TestJwt(t *testing.T) {
	// create rsa keypait for testing
	reader := rand.Reader
	bitSize := 512
	privateKey, _ := rsa.GenerateKey(reader, bitSize)

	handlerFactory := instrumented.NewHandlerFactory("d4l", instrumented.DefaultInstrumentInitOptions, instrumented.DefaultInstrumentOptions)
	auth := NewAuth("", &privateKey.PublicKey, handlerFactory, AuthWithLatencyBuckets([]float64{4, 8, 16}), AuthWithSizeBuckets([]float64{4, 8, 16}))
	...
	tests := []struct {
		...
		{"valid Service Auth", signedToken, http.StatusOK},
		{"invalid Service Auth", "random", http.StatusUnauthorized},
	}
	for _, tt := range tests {
		tt := tt
