...
// Emitter emits business intelligence (BI) events.
type Emitter struct {
	serviceName    string
...
func (e *Emitter) Log(event Event) error {
	...
	return e.out.Encode(baseEvent)
}

// LogCtx wraps Log(event Event) and fills UserID from the request context
// (populated by the log middleware under log.UserIDContextKey) when the caller
// left it empty. SessionID is not derived automatically; set it on the event if
// it is needed.
func (e *Emitter) LogCtx(ctx context.Context, event Event) error {
	if event.UserID == "" {
		if userID, ok := ctx.Value(log.UserIDContextKey).(string); ok {
			event.UserID = userID
		}
	}
